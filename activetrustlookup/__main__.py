...
if len(sys.argv) != 2:
    print "Usage: activetrustlookup <configuration files directory>"
    sys.exit(1)
...
logging_config_path = os.path.join(config_dir, DXLATLookup.LOGGING_CONFIG_FILE)
...
with DXLATLookup(sys.argv[1]) as app:
    try:
        # Run the application
        app.run()
        running = True
